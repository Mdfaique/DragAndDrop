...
// Add event listeners to the containers og2
...
for (var i = 0; i < containers.length; i++) {
  ...
  containers[i].addEventListener("drop", function (event) {
    ...
    if (draggedItem !== null && draggedItem.parentNode !== this) {
      ...
      showMessage(
        itemNumber + " block " + " dropped successfully.",
        "successMsg"
      );
...
// Show a message in the specified container og2
...
function reset() {
  ...
  attachEventListeners(); // Reattach event listeners to the items
}
